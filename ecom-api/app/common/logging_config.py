...
from app.config import ECOM_API_ROOT, get_settings
...
def setup_logging(service: str) -> logging.Logger:
    ...
    if settings.log_format.lower() == "json":
        ...
        text_format = "%(asctime)s %(levelname)s [%(service)s] [%(request_id)s] %(name)s: %(message)s"
    ...
    app_logger = logging.getLogger("ecom")
    app_logger.info("Logging initialized service=%s level=%s format=%s", service, settings.log_level, settings.log_format)
    return app_logger
